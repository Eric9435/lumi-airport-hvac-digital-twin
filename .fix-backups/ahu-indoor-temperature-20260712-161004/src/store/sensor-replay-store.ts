...
function stagePlantForRow(row: SensorCsvRow): void {
  ...
  useEnterprisePlantStore.setState(() => ({
    ...
    predictedCoolingLoadKw: row.calculatedCoolingLoadKw,
    coolingDemandPercent: Math.min(
      100,
      (row.calculatedCoolingLoadKw / (52 * 4)) * 100,
    ),
  ...
  applyRow: (index) => {
    ...
    set({
      ...
      message: `Applied ${new Date(
        row.timestamp,
      ).toLocaleString()} — ${calculateRequiredChillerCount(
        row.calculatedCoolingLoadKw,
      )} chiller(s) required.`,
    });
  },
